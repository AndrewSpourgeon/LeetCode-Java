...
class Solution
{
    ...
    private Pair solve(Node root){
        ...
        Pair lh = solve(root.left);
        Pair rh = solve(root.right);
        
        if(lh.valid && (root.data == lh.val || lh.val == -1)){
            if(rh.valid && (root.data == rh.val || rh.val == -1)){
                ans = ans + 1;
...
class Pair{
    boolean valid;
    int val;
    public Pair(boolean valid , int val){
        this.valid = valid;
        this.val = val;
    }
}